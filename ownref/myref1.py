...
def getHLTprescaledic(file_dataset,file_path,file_prescale,steamFile):
    #file_path=1
    #file_prescale=0
    hltprescaledic={}
    if '.tsv' in steamFile:
        ...
        for Line in filesteam:
            line=Line.split('\t')
            if 'dataset' in line[file_dataset]:
                dataset=line[file_dataset]
            path=line[file_path].split('_v')[0]
            if not 'HLT_' in path:continue
            hltprescaledic[path]=(line[file_prescale],dataset)
...
def getNTuplesPrescale(rootfile,type_str):
    import ROOT
    prescales={}
  ...
  # take the first "hltbit" file                                                                                                   
    ...
    for leaf in chain.GetListOfLeaves():
        name = leaf.GetName()
        if ((evaHLT and ("HLT_" in name)) or (evaL1 and ("L1_" in name))) and not ("Prescl" in name):
            trigger=name
            i=0
            pname=name+'_Prescl'
            for event in chain:
