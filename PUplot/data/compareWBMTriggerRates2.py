...
#******************************************************************************************
file1name = 'HLT.csv'
spreadSheetHeader1="Path,L1Path,L1Prescale("+index_lumi+"),Steam L1path,Steam L1Prescale,totalprescale,WBMhltPrescale("+index_lumi+"),Steam hltPrescale,,Data Rate (Hz),,,Steam Rate (Hz),,,Steam Rate re-hltscaled(Hz),,,Steam rate l1prescaled,,,Bias(data-steam)\n"
...
parser.add_argument('--minLS',help='minimum LS (inclusive)',default=1054,type=int)
parser.add_argument('--maxLS',help='maximum LS (inclusive)',default=1530,type=int)
parser.add_argument('--runnr',help='runnr',default=260627,type=int)
targetLumi = int(index_lumi.split('e')[0])*(10**(int(index_lumi.split('e')[1])-30))
#parser.add_argument('--targetLumi',help='lumi to scale to (units of 1E30, so 7E33 is 7000)',default=2000,type=float)
if P_hltrates:
...
print "run No : ",args.runnr
print "min LS : ",args.minLS
print "max LS : ",args.maxLS
print "target Lumi : ",targetLumi
print "*"*30
if P_hltrates:
    print "steam hlt file : ",args.steamRates
print "*"*30
if P_l1rates:
    print "steam l1 file : ",args.steamL1Rates
    print "*"*30
#print args.steamRates

avePU=getPU(args.runnr,args.minLS,args.maxLS)
...
if P_hltrates:
    ...
    file1name=str(args.runnr)+file1name
...
#        relDiff,relDiffErr=Bias(ratePred,rateScaled,ratePredErr,rateScaledErr)
...
if P_l1rates:
    ...
    file2name=str(args.runnr)+file2name
...
'HLT_DiPFJetAve320']
file3name=str(args.runnr)+file3name
filetowrite3=open(file3name,'w')   #l1 rate comparision output file
filetowrite3.write(spreadSheetHeader1)

for path in testlist:
    for i in range(len(HLTlist)):
        if path in HLTlist[i]:
            filetowrite3.write(spreadSheetStr1 %(path,HLTlist[i][path][0],HLTlist[i][path][1],HLTlist[i][path][2],HLTlist[i][path][3],HLTlist[i][path][4],HLTlist[i][path][5],HLTlist[i][path][6],HLTlist[i][path][7],HLTlist[i][path][8],HLTlist[i][path][9],HLTlist[i][path][10],HLTlist[i][path][11],HLTlist[i][path][12],HLTlist[i][path][13],HLTlist[i][path][14],HLTlist[i][path][15],HLTlist[i][path][16]))
