...
htmfile=open('mc_plot/index.html','w')
...
#    print y
    ...
    gr.GetYaxis().SetTitle( 'Steam Rates MC / total CrossSection (Hz/pb)' )
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    legend.AddEntry(gr,"7e33","lp")#  ,   hlt Prescale : %s"%(str(gdic[run][0][path][5])),"lp")
    legend.AddEntry(gr2,"2e33","lp")#  ,   hlt Prescale : %s"%(str(gdic2[run][0][path][5])),"lp")
    legend.AddEntry(gr3,"1e34","lp")#  ,   hlt Prescale : %s"%(str(gdic2[run][0][path][5])),"lp")
    legend.Draw()
